...
public class RegisterUser : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (step == "1")
        {
            SaveDataStep1(context);
        }
        else if (step == "2")
        {
            SaveDataStep2(context);
        }
        else if (step == "4")
        {
    ...
    private void SaveDataStep1(HttpContext context)
    {
        ...
        userDetail.MembershipLevel = null;//Incomplete
        ...
        polinatorInformation.LandscapeCity = string.Empty;
        polinatorInformation.LandscapeState = string.Empty;
        polinatorInformation.LandscapeZipcode = string.Empty;

        //size, type
        polinatorInformation.PollinatorSize = 9;
        // polinatorInformation.PollinatorType = 9;
    ...
    private void SaveDataStep2(HttpContext context)
    {
        ...
        polinatorInformation.LandscapeCity = context.Request.Form["landscapeCity"].Trim();
        polinatorInformation.LandscapeZipcode = context.Request.Form["landscapeZipcode"].Trim();
        ...
        if (context.Request.Form["notCreateAccount"] == "false" && !String.IsNullOrEmpty(userName))
        {        
            var userAccount = (from u in mydb.Users
                               where u.UserId == UserID
                               select u).FirstOrDefault();
            userAccount.UserName = userName;
            mydb.SaveChanges();
            
           /* var userMembership = (from u in mydb.Memberships
                                  where u.UserId == UserID
                                  select u).FirstOrDefault();
            userMembership.Password = password;*/

           MembershipUser aspNetUser = System.Web.Security.Membership.GetUser(userName);
            string tempPassword = aspNetUser.ResetPassword();
            aspNetUser.ChangePassword(tempPassword, password);
            
            context.Session[UserID.ToString() + "password"] = HttpUtility.HtmlEncode(password);
        }
    ...
    private void SaveDataStep4(HttpContext context)
    {
        ...
        if (polinatorInformation != null)
        {
            ...
            polinatorInformation.Description = context.Request.Form["orgDescription"].Trim();
            polinatorInformation.LastUpdated = DateTime.Now;  
            mydb.SaveChanges();
        }
